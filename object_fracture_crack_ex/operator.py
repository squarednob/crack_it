...
# Access by bpy.ops.mesh.crackit_ex_fracture
class FractureOperation(Operator):
    bl_idname = "mesh.crackit_ex_fracture"
    bl_label = "1.Crack"
    ...
    def execute(self, context):
        if check_object_cell_fracture():
            crackit = context.scene.crackit_ex
            ...
            if crackit.fracture_source == 'PENCIL':
                try:
                    ...
                    crack_it.error_handlers(
                            self, "mesh.crackit_ex_fracture", e, "Detecting Grease Pencil was failed. "
            ...
            try:
                ...
                crack_it.error_handlers(
                        self, "mesh.crackit_ex_fracture", e, "Crack It! could not be completed."
                        )
                return {"CANCELLED"}
        else:
            self.report({'WARNING'},
...
# Access by bpy.ops.mesh.crackit_ex_surface
class SurfaceOperation(Operator):
    bl_idname = "mesh.crackit_ex_surface"
    bl_label = "2.Surface"
    ...
    def execute(self, context):
        if check_object_cell_fracture():
            ...
            try:
                ...
                crack_it.error_handlers(
                        self, "mesh.crackit_ex_surface", e, "Crack It! could not be completed."
                        )
                return {"CANCELLED"}
        else:
            self.report({'WARNING'},
...
# Access by bpy.ops.mesh.crackit_ex_material
class MaterialOperation(Operator):
    bl_idname = "mesh.crackit_ex_material"
    bl_label = "3.Material"
    ...
    def execute(self, context):
        ...
        try:
            ...
            crack_it.error_handlers(
                    self, "mesh.crackit_ex_material", e,
...
class crackitPanel(Panel):
    bl_label = "Crack it! EX"
    bl_idname = 'crack_it_ex'
